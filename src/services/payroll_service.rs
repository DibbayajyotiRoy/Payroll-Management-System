...
use crate::domain::models::{AttendanceRecord, JsonbValue, LeavePolicy, RoleConfiguration};
...
impl BatchPayrollProcessor {
    ...
    pub fn process_batch(
        &self,
        requests: Vec<PayrollCalculationRequest>,
    ) -> Vec<Result<PayrollCalculationResult, PayrollError>> {
        requests
            .into_iter()
...
impl PayrollService {
    ...
    pub async fn create_role(
        &mut self,
        role_config: CreateRoleRequest,
    ) -> Result<RoleConfiguration, PayrollError> {
        let mut conn = self
            .pool
            .get()
            .map_err(|_| PayrollError::DatabaseConnectionError)?;
        let role_id = Uuid::new_v4().to_string();
        let new_role = RoleConfiguration {
            role_id: role_id.clone(),
            role_name: role_config.role_name,
            company_id: role_config.company_id,
            ...
            overtime_policy: JsonbValue(
                to_value(role_config.overtime_policy)
                    .map_err(|_| PayrollError::SerializationError)?,
            ),
            leave_policies: JsonbValue(
                to_value(role_config.leave_policies)
                    .map_err(|_| PayrollError::SerializationError)?,
            ),
    ...
    pub async fn get_role_by_id(
        &self,
        role_id: &str,
    ) -> Result<Option<RoleConfiguration>, PayrollError> {
        let mut conn = self
            .pool
            .get()
            .map_err(|_| PayrollError::DatabaseConnectionError)?;
    ...
    pub fn process_payroll(&self, request: CreatePayrollRequest) -> PayrollResponse {
        ...
        for employee in &request.employees {
            if let Some(role_config) = request
                .role_configs
                .iter()
                .find(|config| config.role_id == employee.role_id)
            {
                let employee_attendance: Vec<AttendanceRecord> = request
                    .attendance_records
                    .iter()
                ...
                match self.processor.engine.calculate(calc_request) {
                    ...
                    Err(error) => {
                        errors.push(format!("Employee {}: {}", employee.employee_id, error))
                    }
                }
            } else {
                errors.push(format!(
                    "Role configuration not found for employee {}: {}",
    ...
    pub fn validate_configuration(&self, config: &RoleConfiguration) -> Vec<String> {
        ...
        let overtime_policy: crate::domain::models::OvertimePolicy =
            serde_json::from_value(config.overtime_policy.0.clone()).unwrap_or_default();
        if overtime_policy.weekday_multiplier <= 0.0
            || overtime_policy.weekend_multiplier <= 0.0
            || overtime_policy.holiday_multiplier <= 0.0
        {
            validation_errors.push("Overtime multipliers must be greater than 0".to_string());
        }

        let leave_policies: Vec<LeavePolicy> =
            serde_json::from_value(config.leave_policies.0.clone()).unwrap_or_default();
        for policy in &leave_policies {
            if policy.deduction.deduction_type == "percent"
                && (policy.deduction.value < 0.0 || policy.deduction.value > 100.0)
            {
                validation_errors.push(format!(
                    "Invalid deduction percentage for leave type {}: must be between 0.0 and 100.0",
